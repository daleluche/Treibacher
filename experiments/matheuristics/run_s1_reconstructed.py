"""Run the reconstructed S_1 experiment suite with resumability."""
...
INSTANCE_PATH = ROOT / "experiments" / "GAMSPy" / "S" / "S_1.py"
RESULTS_DIR = ROOT / "experiments" / "matheuristics" / "results_production" / "s1"
...
def result_path(method: str, budget: float, seed: int, output_dir: Path) -> Path:
    ...
    return output_dir / f"S_1_{method_label(method)}_seed{seed}_b{int(budget)}.json"
...
def params_for(method: str, budget: float, threads: int, log_windows: bool) -> RunParams:
    """Build run parameters for one S_1 job."""
    return RunParams(
        budget=budget,
        sigma=10,
        step=5,
...
def run_ils(output_dir: Path, n_runs: int, time_limit_s: float, base_seed: int, workers: int) -> Path:
    """Run ILS v2 for S_1 and return the summary path."""
    run_instance(
    ...
    return output_dir / "S_1_summary.json"
...
def main(argv: Iterable[str] | None = None) -> int:
    """Run the reconstructed S_1 suite."""
    parser = argparse.ArgumentParser(description="Run reconstructed S_1 production jobs.")
    ...
    for method, budget, log_windows in MATHEURISTIC_JOBS:
        ...
        if not args.force and result_matches(out_path, method, budget, args.seed, args.threads, log_windows):
            ...
            continue
        logging.info("RUN S_1 %s @ %.0fs", method, budget)
    ...
    if not args.skip_ils and (not selected or "ils" in selected or "ILS" in selected):
        summary_path = args.output_dir / "S_1_summary.json"
        if not args.force and ils_summary_matches(summary_path, args.ils_runs, args.ils_time):
            logging.info("SKIP ILS: %s", summary_path)
        else:
            logging.info("RUN S_1 ILS v2: runs=%d time=%.0fs workers=%d", args.ils_runs, args.ils_time, args.ils_workers)
