...
def result_path(instance: str, method: str, seed: int) -> Path:
    ...
    return RESULTS_DIR / f"{instance}_{method_label(method)}_seed{seed}_b3600.json"
...
def build_summary_rows(seed: int) -> list[dict]:
    ...
    for dataset, instance, _ in SCALE_INSTANCES:
        ...
        for method in METHODS:
            path = result_path(instance, method, seed)
            if path.exists():
                data = json.loads(path.read_text(encoding="utf-8"))
...
def write_outputs(rows: list[dict]) -> tuple[Path, Path]:
    ...
    for row in rows:
        lines.append(
            "| {instance} | {rf_fo} | {rf_mip} | {mip} | {best_method} | {best_z} |".format(
...
def print_table(rows: list[dict]) -> None:
    ...
    columns = ["instance", "Z_rf_fo", "Z_rf_mip", "Z_mip", "best_method", "best_Z"]
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    for idx, (dataset, instance, path, method) in enumerate(planned, start=1):
        ...
        written = run_one(path, method, args.budget, args.seed, args.threads)
        print(f"[DONE] {label}: {written}")

    rows = build_summary_rows(args.seed)
    csv_path, report_path = write_outputs(rows)
    print_table(rows)
