\begin{table}[t]
\centering
\footnotesize
\caption{Benchmark dimensions by instance family. Instance Ale$_1$ has a longer horizon and one additional process; see Section~\ref{sec:difficulty}.}
\label{tab:instances}
\resizebox{\linewidth}{!}{%
\begin{tabular}{lrrrrrrr}
\toprule
Set & Inst. & $T$ & $J$ & $I$ & Binary vars. & Continuous vars. & Constraints \\
\midrule
Real ($\mathrm{Ale}_2$--$\mathrm{Ale}_{10}$) & 9 & 19 & 159 & 50 & 3\,021 & 1\,901 & 970 \\
Real ($\mathrm{Ale}_1$) & 1 & 25 & 160 & 50 & 4\,000 & 2\,501 & 1\,276 \\
2X & 10 & 38 & 159 & 50 & 6\,042 & 3\,801 & 1\,939 \\
3X & 10 & 57 & 159 & 50 & 9\,063 & 5\,701 & 2\,908 \\
4X & 10 & 76 & 159 & 50 & 12\,084 & 7\,601 & 3\,877 \\
5X & 10 & 95 & 159 & 50 & 15\,105 & 9\,501 & 4\,846 \\
8X & 5 & 152 & 159 & 50 & 24\,168 & 15\,201 & 7\,753 \\
10X & 5 & 190 & 159 & 50 & 30\,210 & 19\,001 & 9\,691 \\
\bottomrule
\end{tabular}%
}
\end{table}
