...
which the PSP is embedded (\S\ref{sec:rw-taxonomy}); applications
featuring process selection and co-production in fixed proportions
(\S\ref{sec:rw-coproduction}); MIP-based matheuristics of the
relax-and-fix and fix-and-optimize families
(\S\ref{sec:rw-matheuristics}); and the solver-progress and
experimental-methodology literature that underpins our protocol
(\S\ref{sec:rw-methodology}).
...
solution warm-starts the \emph{monolithic} model -- a hybrid that is,
by construction, never worse than the cold-started solver under equal
budgets.
